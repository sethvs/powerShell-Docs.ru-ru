...
В этой статье описаны специальные возможности [!INCLUDE[ise_1](../Token/ise_1_md.md)], которые могут вам пригодиться.
...
Корпорация Майкрософт стремится стремится сделать использование своих продуктов и услуг простым для всех пользователей. В следующих разделах представлена информация о функциях, продуктах и услугах, повышающих доступность [!INCLUDE[ise_2](../Token/ise_2_md.md)] для пользователей с ограниченными возможностями.

[!INCLUDE[ise_2](../Token/ise_2_md.md)] поддерживает режим высокой контрастности. Для пользователей с ослабленным зрением сведения о точках останова доступны через командлеты для управления точками останова, например [Get-PSBreakpoint](assetId:///0bf48936-00ab-411c-b5e0-9b10a812a3c6) и [Set-PSBreakpoint](assetId:///6afd5d2c-a285-4796-8607-3cbf49471420). Дополнительные сведения см. в разделе "Управление точками останова" статьи [Отладка сценариев в интегрированной среде сценариев Windows PowerShell](../Topic/How-to-Debug-Scripts-in-Windows-PowerShell-ISE.md#bkmk_1). Помимо специальных возможностей и служебных программ в составе Microsoft Windows, следующие функции обеспечивают легкий доступ к интегрированной среде сценариев Windows PowerShell для пользователей с физическими ограничениями:

-   Сочетания клавиш

-   Таблица цветовой подсветки синтаксиса и возможность изменять некоторые другие параметры цветов с помощью объекта сценария [$psISE.Options](assetId:///75e2a76f-f3d1-490b-ad5d-e3829946aabb).

-   Изменение размера текста

## <a name="bkmk_1"></a> Изменение размера и расположения областей консоли и сценариев
Следующие шаги можно использовать для изменения размера и расположения областей консоли и сценариев. При повторном открытии [!INCLUDE[ise_2](../Token/ise_2_md.md)] изменения в размере и расположении сохранятся.
...
-   Чтобы поместить область сценариев над областями команд и вывода, нажмите клавиши **CTRL+1**, щелкните значок **Показать область сценариев сверху** в меню **Вид** или выберите **Показать область сценариев сверху**.
...
|Действие|Сочетание клавиш|
|----------|---------------------|
|**Переход в область команд**|CTRL+D|
|**Переход в область вывода**|CTRL+SHIFT+O|
...
|Действие|Сочетание клавиш|Область использования|
|----------|---------------------|----------|
|**Продолжение**|C|Область сценариев при отладке сценария|
|**Шаг с заходом**|S|Область сценариев при отладке сценария|
|**Шаг с обходом**|V|Область сценариев при отладке сценария|
|**Шаг с выходом**|O|Область команд при отладке сценария|
|**Повтор последней команды** (для шага с заходом или шага с обходом)|ВВОД|Область команд при отладке сценария|
|**Отображение стека вызовов**|K|Область команд при отладке сценария|
|**Остановка отладки**|Q|Область команд при отладке сценария|
|**Вывод сценария**|L|Область команд при отладке сценария|
|**Отображение команд отладки для консоли**|H или ?|Область команд при отладке сценария|
...
Для запуска консоли PowerShell (PowerShell.exe) или выхода из [!INCLUDE[mshgraphicalhostshort](../Token/mshgraphicalhostshort_md.md)] можно использовать приведенные ниже сочетания клавиш.
