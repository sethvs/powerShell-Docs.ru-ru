...
При прохождении проверки подлинности на других компьютерах контекстом пользователя становится локальная учетная запись компьютера, а не виртуальная учетная запись.

На контроллере домена Active Directory виртуальные учетные записи по умолчанию получают права *администратора домена*.
Это связано с отсутствием группы локальных администраторов на контроллере домена.
Соответственно, виртуальные учетные записи на контроллерах домена являются учетными записями домена и могут использоваться на других компьютерах.
При настройке сеансов JEA на контроллере домена следует соблюдать осторожность, чтобы избежать предоставления команд, которые можно использовать для управления другими компьютерами в сети.
...
-----------------------------|-------------------------------------|---------------------------------------------------------|--------------------------------------------------
Контроллер домена            | Значение по умолчанию                             | Пользователь домена, член группы "*DOMAIN*\Domain Admins"         | Пользователь домена, член группы "*DOMAIN*\Domain Admins"
Контроллер домена            | Группы доменов A и B               | Пользователь домена, член групп "*DOMAIN*\A", "*DOMAIN*\B"       | Пользователь домена, член групп "*DOMAIN*\A", "*DOMAIN*\B"
...
**Сквозные учетные данные** — если не указать учетную запись запуска от имени, PowerShell использует учетные данные подключающегося пользователя для выполнения команд на удаленном сервере.
