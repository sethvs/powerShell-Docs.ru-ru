...
>Примечание. Сервер отчетов, описанный в этом разделе, не доступен в PowerShell 4.0. Сведения о создании отчетов в PowerShell 4.0 см. в разделе "Использование сервера соответствия DSC".
...
Для настройки отправки отчетов на сервер на узле используется блок ReportServerWeb в конфигурации LCM узла (сведения о настройке LCM
см. в разделе Настройка локального диспетчера конфигураций). Сервер, на который узел отправляет отчеты, необходимо настроить как опрашивающий веб-сервер (отправлять отчеты
в общую папку SMB невозможно). Сведения о настройке опрашивающего сервера см. в разделе Настройка опрашивающего веб-сервера DSC. Сервер отчетов может быть той же службой, в которой
...
где `MyNodeAgentId` — это AgentId узла, с которого вы хотите получать отчеты. Вы можете получить AgentID узла, вызвав Get-DscLocalConfigurationManager
на этом узле.
