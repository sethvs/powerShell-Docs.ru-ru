...
При выборе названия для записи DNS следует учитывать архитектуру решения. Если используется балансировка нагрузки, название сертификата, применяемого для защиты трафика по протоколу HTTPS, должно совпадать с названием записи DNS. Аналогично при использовании общей папки с высоким уровнем доступности будет применяться виртуальное название кластера.
...
Развертывание с высоким уровнем доступности |Если клиенты будут подключаться через решение по балансировке нагрузки, создайте имя узла для виртуального IP-адреса и запись CNAME, которая ссылается на это имя. Если для распределения запросов от клиентов между опрашивающими серверами будет использоваться циклический перебор DNS, необходимо настроить записи имен и включить в них имена узлов всех развернутых экземпляров опрашивающих серверов.|
...
#### <a name="designing-for-high-availability"></a>Проектирование для обеспечения высокого уровня доступности  
Роль опрашивающего сервера можно развернуть в высокодоступной архитектуре. Для роли веб-службы может быть сбалансирована нагрузка, а файлы и папки, которые содержат модули DSC и конфигурации DSC, могут находиться в хранилище с высоким уровнем доступности.

Необходимо иметь в виду, что после доставки конфигураций и модулей на целевой узел все данные, необходимые для выполнения тестов и задания конфигураций, сохраняются локально на каждом узле. С опрашивающего сервера доставляются только изменения. Недоступность опрашивающего сервера будет являться прерыванием только когда развертывания активны.  Как правило, высокий уровень доступности гарантируется только для самой крупной среды.

При настройке опрашивающих серверов с высоким уровнем доступности требуется продумать процессы распространения запросов от клиентов среди нескольких узлов сервера и предоставления общего доступа к необходимым серверным файлам на этих узлах.
...
При обеспечении высокого уровня доступности, когда несколько серверов настроены как опрашивающие серверы и между ними установлены подключения с балансировкой нагрузки, очень важно обеспечить с этих серверов доступ к одинаковым ресурсам и конфигурациям. Лучший способ сделать это — сохранить нужное содержимое в расположении с высоким уровнем доступности, таком как кластеризованная общая папка. Расположение с папкой можно указать в конфигурации каждого сервера. Дополнительные сведения о вариантах хранения общих данных см. в статье "Обзор масштабируемого файлового сервера для данных приложений".

Задача по планированию|
---|
Какое решение будет использоваться для размещения общей папки с высоким уровнем доступности?|
Кто будет обрабатывать запрос на новую общую папку с высоким уровнем доступности?|
Каково среднее время получения доступа к общей папке с высоким уровнем доступности?|
Какие данные потребуются рабочим группам, отвечающим за хранение данных и (или) кластеризацию?|
