...
Кроме того, ресурсы можно распределить между несколькими файлами `.psm1` и включить их в виде вложенных модулей.
Это целесообразно, если ресурсов много и размещение их в одном файле усложнит процесс управления.
...
В один файл можно поместить как один, так и несколько классов. 
Ресурсы удобно группировать по дочерней области в пределах одного вложенного модуля.
При этом с точки зрения пользователя работа с ресурсами никак не изменится.
Все ресурсы будут отображаться в модуле `MyDscResource`.
Вложенные модули можно рассматривать как элементы реализации, предназначенные для вашего удобства.
...
Чтобы сделать ресурс на основе класса доступным для модуля DSC, необходимо включить оператор **DscResourcesToExport** в файле манифеста, который указывает модулю, что нужно экспортировать ресурсы. 
...
Если вы используете **вложенные модули** для распределения ресурсов между несколькими файлами, занесите список вложенных модулей в ключ `NestedModules`.
